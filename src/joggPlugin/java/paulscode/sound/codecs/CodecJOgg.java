 ...
 * J-Ogg License:
 *
 * You are free to use, modify, redistribute or include this software in your
 ...
 * https://www.j-ogg.de and copyrighted by Tor-Einar Jarnbjo."
 *
...
// From the j-ogg library, https://www.j-ogg.de
...
public class CodecJOgg implements ICodec {
	...
	public boolean initialize(URL url) {
		...
		try {
			...
			myIdentificationHeader = myVorbisStream.getIdentificationHeader();

			// Set up the audio format to use during playback:
			myAudioFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, (float) myIdentificationHeader.getSampleRate(), 16, myIdentificationHeader.getChannels(), myIdentificationHeader.getChannels() * 2, (float) myIdentificationHeader.getSampleRate(), true);
	...
	public SoundBuffer read() {
		...
		if (reverseBytes) reverseBytes(streamBuffer, 0, bytesRead);

		// If we didn't fill the stream buffer entirely, trim it down to size:
		if (bytesRead < streamBuffer.length) streamBuffer = trimArray(streamBuffer, bytesRead);
	...
	public SoundBuffer readAll() {
		...
		if (fileSize > 0) {
			...
			while ((!endOfStream(GET, XXX)) && (totalBytes < SoundSystemConfig.getMaxFileSize())) {
				...
				if (reverseBytes) reverseBytes(smallBuffer, 0, bytesRead);
		...
		// Insert the converted data into a ByteBuffer
		byte[] data = null;
		if (fullBuffer != null) data = convertAudioBytes(fullBuffer, myAudioFormat.getSampleSizeInBits() == 16);
	 ...
	 */
	public void cleanup() {
		if (myLogicalOggStream != null) try {
			myLogicalOggStream.close();
		} catch (Exception ignored) {
		}
		if (myVorbisStream != null) try {
			myVorbisStream.close();
		} catch (Exception ignored) {
		}
		if (myOggInputStream != null) try {
			myOggInputStream.close();
		} catch (Exception ignored) {
		}
		if (myAudioInputStream != null) try {
			myAudioInputStream.close();
		} catch (Exception ignored) {
		}
	...
	/**
	 * The OggInputStream class provides an InputStream interface for reading from a .ogg file.
	 */
	private static class OggInputStream extends InputStream {
		/**
		 * The VorbisStream to read from.
		 */
		private final VorbisStream myVorbisStream;
