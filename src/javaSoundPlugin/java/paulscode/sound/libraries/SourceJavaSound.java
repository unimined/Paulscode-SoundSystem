...
public class SourceJavaSound extends Source {
	...
	public void changeSource(boolean priority, boolean toStream, boolean toLoop, String sourceName, FilenameURL filenameURL, SoundBuffer soundBuffer, float x, float y, float z, int attModel, float distOrRoll, boolean temporary) {
		...
		if (channelJavaSound != null) channelJavaSound.setLooping(toLoop);
		positionChanged();
	}
	...
	public void play(Channel c) {
		...
		if (newChannel) {
			...
			if (!toStream) {
				...
				if (channelJavaSound != null) channelJavaSound.attachBuffer(soundBuffer);
			}
		}
		...
		if (nextCodec != null && !noNextBuffers) {
			...
			synchronized (soundSequenceLock) {
				while (!nextBuffers.isEmpty()) {
					...
					if (soundBuffer != null && soundBuffer.audioData != null) preLoadBuffers.add(soundBuffer.audioData);
				}
			}
		} else {
		...
		switch (attModel) {
			...
			case SoundSystemConfig.ATTENUATION_ROLLOFF:
				if (distanceFromListener <= 0) {
					...
					// (roll-off should NEVER be negative)
	...
	public void calculatePan() {
		...
		if (channel != null && channel.attachedSource == this && channelJavaSound != null) {
			if (attModel == SoundSystemConfig.ATTENUATION_NONE) channelJavaSound.setPan(0);
			else channelJavaSound.setPan(pan);
		}
	}
	...
	/**
	 * @deprecated use {@link Math#min(float, float)} instead
	 */
	@Deprecated
