...
public class LibraryJavaSound extends Library {
		...
		if (myMixer == null) {
			...
			for (Mixer.Info mixerInfo : AudioSystem.getMixerInfo()) {
				if (mixerInfo.getName().equals("Java Sound Audio Engine")) {
					...
					if (mixerRanker.rank < 14) break;  // Minor problem, see if there is a better mixer
			...
			if (myMixer == null) {
				...
				for (Mixer.Info mixerInfo : AudioSystem.getMixerInfo()) {
					...
					try {
						// See how good it is
						mixerRanker.rank(mixerInfo);
					} catch (LibraryJavaSound.Exception ignored) {
					}
	...
	public static boolean libraryCompatible() {
		...
		for (Mixer.Info mixerInfo : AudioSystem.getMixerInfo()) {
			if (mixerInfo.getName().equals("Java Sound Audio Engine")) return true;
		}
		return false;
	}
	...
	public boolean loadSound(FilenameURL filenameURL) {
		...
		codec.cleanup();
		codec = null;
		if (buffer != null) bufferMap.put(filenameURL.getFilename(), buffer);
		else errorMessage("Sound buffer null in method 'loadSound'");
	...
	public boolean loadSound(SoundBuffer buffer, String identifier) {
		...
		// save it for later:
		if (buffer != null) bufferMap.put(identifier, buffer);
		else errorMessage("Sound buffer null in method 'loadSound'");
	...
	private static synchronized MixerRanking mixerRanking(boolean action, MixerRanking value) {
		if (action == SET) myMixerRanking = value;
	...
	private static synchronized int minSampleRate(boolean action, int value) {
		if (action == SET) minSampleRate = value;
	...
	private static synchronized int maxSampleRate(boolean action, int value) {
		if (action == SET) maxSampleRate = value;
	...
	private static synchronized boolean useGainControl(boolean action, boolean value) {
		if (action == SET) useGainControl = value;
	...
	private static synchronized boolean usePanControl(boolean action, boolean value) {
		if (action == SET) usePanControl = value;
	...
	private static synchronized boolean useSampleRateControl(boolean action, boolean value) {
		if (action == SET) useSampleRateControl = value;
	...
	public static String getDescription() {
		return "The Java Sound API.  For more information, see "
				+ "https://web.archive.org/web/20100628094942/https://java.sun.com/products/java-media/sound";
	}
