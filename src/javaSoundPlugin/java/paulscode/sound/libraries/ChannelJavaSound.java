 ...
 * JavaSound.
 * Channels can be either normal or streaming channels.
...
public class ChannelJavaSound extends Channel {
	...
	// END NORMAL SOURCE VARIABLES

	// STREAMING SOURCE VARIABLES:
	/**
	...
	// END STREAMING SOURCE VARIABLES:
	...
	/**
	 * The initial decibel change (start at normal volume).
	 */
	...
	/**
	 * Takes channelType identifier and a handle to the Mixer as parameters.
	...
	public void newMixer(Mixer m) {
		if (myMixer != m) {
			try {
				...
				else if (sourceDataLine != null) sourceDataLine.close();
			} catch (SecurityException ignored) {
			}

			myMixer = m;
			if (attachedSource != null) {
				if (channelType == SoundSystemConfig.TYPE_NORMAL && soundBuffer != null) attachBuffer(soundBuffer);
				else if (myFormat != null) resetStream(myFormat);
			}
		}
	}
	...
	public boolean attachBuffer(SoundBuffer buffer) {
		...
		if (errorCheck(myMixer == null, "Mixer null in method 'attachBuffer'")) return false;

		// make sure the buffer exists:
		if (errorCheck(buffer == null, "Buffer null in method 'attachBuffer'")) return false;
		...
		if (errorCheck(newClip == null, "New clip null in method 'attachBuffer'")) return false;
		...
		// Update the clip and format variables:
		clip = newClip;
		soundBuffer = buffer;
	...
	public boolean resetStream(AudioFormat format) {
		// make sure the Mixer exists:
		if (errorCheck(myMixer == null, "Mixer null in method 'resetStream'")) return false;

		// make sure a format was specified:
		if (errorCheck(format == null, "AudioFormat null in method 'resetStream'")) return false;
		...
		if (errorCheck(newSourceDataLine == null, "New SourceDataLine null in method 'resetStream'")) return false;
	...
	private void resetControls() {
		switch (channelType) {
			case SoundSystemConfig.TYPE_NORMAL:
				...
				try {
					if (!clip.isControlSupported(FloatControl.Type.PAN)) panControl = null;
					else
			...
			case SoundSystemConfig.TYPE_STREAMING:
				// Check if panning is supported:
				try {
					if (!sourceDataLine.isControlSupported(FloatControl.Type.PAN)) panControl = null;
					else
						// Create a new pan Control:
	...
	public void setGain(float g) {
		...
		// convert the supplied linear gain into a "decibel change" value
		// minimumDB is no volume
		// maximumDB is maximum volume
		// (Number of decides is a logarithmic function of linear gain)
	...
	public void setPitch(float p) {
		...
		if (sampleRate < 0.5f) sampleRate = 0.5f;
		if (sampleRate > 2.0f) sampleRate = 2.0f;
	...
	public boolean preLoadBuffers(LinkedList<byte[]> bufferList) {
		...
		if (errorCheck(sourceDataLine == null, "SourceDataLine null in method 'preLoadBuffers'.")) return false;

		sourceDataLine.start();

		if (bufferList.isEmpty()) return true;

		// preload one stream buffer worth of data:
		byte[] preLoad = bufferList.remove(0);

		// Make sure we have some data:
		if (errorCheck(preLoad == null, "Missing sound-bytes in method 'preLoadBuffers'.")) return false;
		assert preLoad != null;

		// If we are using more than one stream buffer, preload the remaining ones now:
		while (!bufferList.isEmpty()) {
			streamBuffers.add(new SoundBuffer(bufferList.remove(0), myFormat));
		}

		// Preload the first stream buffer into the data line:
	...
	public boolean queueBuffer(byte[] buffer) {
		...
		// Make sure we have a SourceDataLine:
		if (errorCheck(sourceDataLine == null, "SourceDataLine null in method 'queueBuffer'.")) return false;

		// make sure a format was specified:
		if (errorCheck(myFormat == null, "AudioFormat null in method 'queueBuffer'")) return false;
	 ...
	 * This method is run from the separate {@link StreamThread StreamThread}.
	 *
	...
	public boolean processBuffer() {
		...
		// Make sure we have a SourceDataLine:
		if (errorCheck(sourceDataLine == null, "SourceDataLine null in method 'processBuffer'.")) return false;

		if (streamBuffers == null || streamBuffers.isEmpty()) return false;

		// Dequeue a buffer and feed it to the SourceDataLine:
		SoundBuffer nextBuffer = streamBuffers.remove(0);

		sourceDataLine.write(nextBuffer.audioData, 0, nextBuffer.audioData.length);
		if (!sourceDataLine.isActive()) sourceDataLine.start();
		nextBuffer.cleanup();
	...
	public int feedRawAudioData(byte[] buffer) {
		...
		if (errorCheck(streamBuffers == null, "StreamBuffers queue null in method 'feedRawAudioData'.")) return -1;
	...
	public int buffersProcessed() {
		...
		// Stream buffers can only be queued for streaming sources:
		if (errorCheck(channelType != SoundSystemConfig.TYPE_STREAMING, "Buffers may only be queued for streaming sources.")) {
			if (streamBuffers != null) streamBuffers.clear();
			return 0;
		}
	...
	public void flush() {
		...
		// Make sure we have a SourceDataLine:
		if (errorCheck(sourceDataLine == null, "SourceDataLine null in method 'flush'.")) return;
		...
		switch (channelType) {
			case SoundSystemConfig.TYPE_NORMAL:
				if (clip != null) {
					...
					clip.setFramePosition(0);
					if (rePlay) {
						if (toLoop) clip.loop(Clip.LOOP_CONTINUOUSLY);
						else clip.start();
					}
				}
				break;
	...
	public float millisecondsPlayed() {
		switch (channelType) {
			...
			case SoundSystemConfig.TYPE_STREAMING:
				if (sourceDataLine == null) return -1;
