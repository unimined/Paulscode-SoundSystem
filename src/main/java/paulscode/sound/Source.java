...
public class Source {
	...
	/**
	 * Determines whether this is a priority source.  Priority sources will
	 * not be overwritten by other sources when there are no available channels.
	 */
	public boolean priority;

	/**
	 * Whether this source should be streamed.
	 */
	 ...
	 * More than one source can not have the same source name.
	 */
	...
	/**
	 * Either fade distance or roll-off factor, depending on the value of attModel.
	 */
	 ...
	 *
	 * @see <a href="https://github.com/MinecraftForge/MinecraftForge/pull/4765">MinecraftForge#4765</a>
	 */
	...
	/**
	 * Whether this source has been paused.
	 */
	...
	/**
	 * Used by streaming sources to indicate whether the initial
	...
	/**
	 * Specifies the number of milliseconds it should take to fade out.
	 */
	protected long fadeOutMilis = 0;

	/**
	 * Specifies the number of milliseconds it should take to fade in.
	 */
	protected long fadeInMilis = 0;

	/**
	 * System time in milliseconds when the last fade in/out volume check occurred.
	 */
	 ...
	 * @param distOrRoll  Either the fading distance or roll-off factor, depending on the value of 'att'.
	 * @param temporary   Whether to remove this source after it finishes playing.
	 */
	public Source(boolean priority, boolean toStream, boolean toLoop, String sourceName, FilenameURL filenameURL, SoundBuffer soundBuffer, float x, float y, float z, int attModel, float distOrRoll, boolean temporary) {
		...
		if (toStream && filenameURL != null) codec = SoundSystemConfig.getCodec(filenameURL.getFilename());
	}
	...
	public void cleanup() {
		...
		synchronized (soundSequenceLock) {
			if (soundSequenceQueue != null) soundSequenceQueue.clear();
	...
	public void queueSound(FilenameURL filenameURL) {
		...
		synchronized (soundSequenceLock) {
			if (soundSequenceQueue == null) soundSequenceQueue = new LinkedList<>();
	 ...
	 * simply fade out and stop.  The milliseconds parameter must be non-negative or
	 ...
	 * level.  The filenameURL parameter may not be null or empty.  The milliseconds
	...
	public void fadeOutIn(FilenameURL filenameURL, long millisOut, long millisIn) {
		...
		if (millisOut < 0 || millisIn < 0) {
			errorMessage("Milliseconds may not be negative in method 'fadeOutIn'.");
			return;
		}

		fadeOutMilis = millisOut;
		fadeInMilis = millisIn;
	...
	public boolean checkFadeOut() {
		...
		if (fadeOutGain == -1.0f && fadeInGain == 1.0f) return false;

		long currentTime = System.currentTimeMillis();
		long millisPast = currentTime - lastFadeCheck;
		...
		if (fadeOutGain >= 0.0f) {
			if (fadeOutMilis == 0) {
				...
				float fadeOutReduction = ((float) millisPast) / ((float) fadeOutMilis);
				fadeOutGain -= fadeOutReduction;
				if (fadeOutGain <= 0.0f) {
					fadeOutGain = -1.0f;
					fadeInGain = 0.0f;
					if (!incrementSoundSequence()) stop();
					positionChanged();
					preLoad = true;
					return false;
				}
		...
		if (fadeInGain < 1.0f) {
			...
			if (fadeInMilis == 0) {
				...
				float fadeInIncrease = ((float) millisPast) / ((float) fadeInMilis);
	...
	public boolean incrementSoundSequence() {
		...
		synchronized (soundSequenceLock) {
			if (soundSequenceQueue != null && !soundSequenceQueue.isEmpty()) {
	 ...
	 * queue, to reduce lag when the transition occurs.  This method has no effect
		...
		synchronized (soundSequenceLock) {
			if (soundSequenceQueue != null && !soundSequenceQueue.isEmpty()) {
				...
				for (int i = 0; i < SoundSystemConfig.getNumberStreamingBuffers() && !nextCodec.endOfStream(); i++) {
					...
					if (buffer != null) {
						if (nextBuffers == null) nextBuffers = new LinkedList<>();
	...
	public int getSoundSequenceQueueSize() {
		if (soundSequenceQueue == null) return 0;
		return soundSequenceQueue.size();
	}

	/**
	 * Sets whether this source should be removed when it finishes playing.
	 *
	...
	/**
	 * Sets whether this source is a priority source.  A priority source
	 * will not be overwritten by another source if there are no channels available
	 * to play on.
	 *
	...
	/**
	 * Sets this source's fade distance or roll-off factor, depending on the
	 * attenuation model.
	 *
	 * @param dr New value for fade distance or roll-off factor.
	 */
	...
	/**
	 * Indicates whether this source's associated library requires some
	...
	public boolean preLoad() {
		...
		synchronized (soundSequenceLock) {
			if (nextBuffers == null || nextBuffers.isEmpty()) noNextBuffers = true;
		}
	...
	public void pause() {
		...
		else errorMessage("Channel null in method 'pause'");
	}
	...
	public void stop() {
		...
		else errorMessage("Channel null in method 'stop'");
	}
	...
	public void rewind() {
		...
		} else errorMessage("Channel null in method 'rewind'");
	}
	...
	public void flush() {
		...
		else errorMessage("Channel null in method 'flush'");
	}
	 ...
	 */
	public void cull() {
		if (!active(GET, XXX)) return;
		if (playing() && toLoop) toPlay = true;
		if (rawDataStream) toPlay = true;
		active(SET, false);
	...
	public boolean playing() {
		if (channel == null || channel.attachedSource != this) return false;
		else if (paused() || stopped()) return false;
		else return channel.playing();
	}
	...
	/**
	 * Returns the number of milliseconds since the source began playing.
	 *
	 * @return milliseconds, or -1 if not playing or unable to calculate
	 */
	public float millisecondsPlayed() {
		if (channel == null) return (-1);
		else return channel.millisecondsPlayed();
	}

	/**
	 * Gets or sets whether the source has been culled
	 *
	 * @param action GET or SET.
	 * @param value  New value if action == SET, or XXX if action == GET.
	 * @return true if the source has been culled
	 */
	private synchronized boolean active(boolean action, boolean value) {
		if (action == SET) active = value;
		return active;
	}

	/**
	 * Sets or returns whether the source has been stopped.
	 *
	 * @return whether the source has been stopped
	 */
	private synchronized boolean stopped(boolean action, boolean value) {
		if (action == SET) stopped = value;
		return stopped;
	}

	/**
	 * Sets or returns whether the source has been paused.
	 *
	 * @return True or False
	 */
	private synchronized boolean paused(boolean action, boolean value) {
		if (action == SET) paused = value;
		return paused;
	}
	...
	public String getClassName() {
		...
		if (libTitle == null || "No Sound".equals(libTitle)) return "Source";
		else return "Source" + libTitle;
	}
