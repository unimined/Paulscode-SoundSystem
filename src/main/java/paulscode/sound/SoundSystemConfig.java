 ...
 * and to link with external plugins. All members of this class are static.
...
//  GLOBAL IDENTIFIERS
	 ...
	 * remains constraint regardless of distance.
	 */
	public static final int ATTENUATION_NONE    = 0;  // no attenuation
	/**
	 * Global identifier for roll-off attenuation.  Roll-off attenuation is a
	 * realistic attenuation model, which uses a roll-off factor to determine how
	 * quickly a source fades with distance.  A smaller roll-off factor will fade at
	 * a further distance, and a roll-off factor of 0 will never fade.  NOTE: In
	 * OpenAL, roll-off attenuation only works for monotone sounds.
	 */
	public static final int ATTENUATION_ROLLOFF = 1;  // logarithmic attenuation
	/**
	 * Global identifier for linear attenuation.  Linear attenuation is less
	 * realistic than roll-off attenuation, but it allows the user to specify a
...
//  PRIVATE STATIC VARIABLES
	 ...
	 * overriding the {@link paulscode.sound.SoundSystemLogger SoundSystemLogger}
	...
	/**
	 * Default value to use for the roll-off factor if not specified.
	 */
	...
	/**
	 * Number of buffers used for each streaming source.  Slow codecs may require
	...
	/**
	 * Indicates whether there is a codec for reading from MIDI files.  If
	 * there is no codec for MIDI, then SoundSystem uses javax.sound.midi.
	 */
	private static boolean midiCodec = false;

	/**
	 * MIDI device to try using as the Synthesizer.  It may be the full name or part
...
// THESE TWO METHODS PROVIDE INFORMATION ABOUT THE INDIVIDUAL SOUND LIBRARIES
	 ...
	 *
	 * @param libraryClass Derivative of class 'Library'.
	 */
	public static void addLibrary(Class<?> libraryClass) throws SoundSystemException {
		...
		if (!libraries.contains(libraryClass)) libraries.add(libraryClass);
	}
	...
	public static void removeLibrary(Class<?> libraryClass) {
		if (libraries == null || libraryClass == null) return;
	...
	public static boolean libraryCompatible(Class<?> libraryClass) {
		if (libraryClass == null) {
			errorMessage("Parameter 'libraryClass' null in method 'libraryCompatible'");
			return false;
		}
		if (!Library.class.isAssignableFrom(libraryClass)) {
	...
	/**
	 * Returns whether it requires reversal of audio data byte-order.
	 *
	 * @param libraryClass Derivative of class 'Library'.
...
// STATIC NONSYNCHRONIZED INTERFACE METHODS
	 ...
	 * extended and methods overridden to change how messages are handled.  Then,
...
//  STATIC SYNCHRONIZED INTERFACE METHODS
	 ...
	 * NOTE: Some sound library plugins may require the total number of channels
	...
	/**
	 * Sets the variable used for overall volume, affecting all sources.
	 *
	...
	/**
	 * Sets the default roll-off factor to use when one is not specified.
	 *
	...
	/**
	 * Returns the default roll-off factor used when one is not specified.
	 *
	 * @return Default roll-off factor
	 */
	...
	/**
	 * Sets the number of buffers used for each streaming source.
	...
	/**
	 * Returns the number of buffers used for each streaming source.
	 *
	...
	/**
	 * Returns whether all sounds in each streaming source's queue will be
	...
	public static synchronized void setCodec(String extension, Class<?> iCodecClass) throws SoundSystemException {
		...
		while (i.hasNext()) {
			codec = i.next();
			if (extension.matches(codec.extensionRegX)) i.remove();
		}
		...
		if (extension.matches(EXTENSION_MIDI)) midiCodec = true;
	}
	...
	public static synchronized ICodec getCodec(String filename) {
		...
		while (i.hasNext()) {
			codec = i.next();
			if (filename.matches(codec.extensionRegX)) return codec.getInstance();
		}
	 ...
	 * there is no codec for MIDI, then SoundSystem uses javax.sound.midi.
	 *
	 * @return True if there is a user defined a MIDI codec.
	 */
	...
	public static void addStreamListener(IStreamListener streamListener) {
		synchronized (streamListenersLock) {
			if (streamListeners == null) streamListeners = new LinkedList<>();
	...
	public static void notifyEOS(String sourceName, int queueSize) {
		synchronized (streamListenersLock) {
			if (streamListeners == null) return;
		}
		...
		new Thread(() -> {
			synchronized (streamListenersLock) {
				...
				while (i.hasNext()) {
					...
					if (streamListener == null) i.remove();
					else streamListener.endOfStream(srcName, qSize);
				}
			}
		}).start();
	}
...
//  PRIVATE INTERNAL METHODS
	...
	private static void errorMessage(String message) {
		if (logger != null) logger.errorMessage("SoundSystemConfig", message, 0);
	}
...
//  PRIVATE INTERNAL CLASSES
	...
	private static class Codec {
		 ...
		 *
		 * @param extension   File extension to be associated with the specified codec.
		 * @param iCodecClass Codec to use for files with the specified extension.
		 */
		public Codec(String extension, Class<?> iCodecClass) {
			...
			if (extension != null && !extension.isEmpty()) {
				...
				for (int x = 0; x < extension.length(); x++) {
					...
					extensionBuilder.append("[").append(c.toLowerCase(Locale.ENGLISH)).append(c.toUpperCase(Locale.ENGLISH)).append("]");
				}
				// The extension will be at the end of the filename:
				extensionRegX += extensionBuilder + "$";
			}
		...
		public ICodec getInstance() {
			if (iCodecClass == null) return null;
