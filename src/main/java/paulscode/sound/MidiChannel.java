...
public class MidiChannel implements MetaEventListener {
	...
	public void fadeOut(FilenameURL filenameURL, long millis) {
		if (millis < 0) {
			errorMessage("Milliseconds may not be negative in method " +
					"'fadeOut'.");
			return;
		}
	...
	/**
	 * @deprecated use {@link #setSourceName(String)} instead
	 */
	@Deprecated
	...
	private synchronized String sourceName(boolean action, String value) {
		if (action == SET) sourceName = value;
		return sourceName;
	}
	 ...
	 *
	 * @param message Meta message describing the MIDI event.
	 */
	...
	private void getSynthesizer() {
		...
		if (overrideMIDISynthesizer != null
				&& !overrideMIDISynthesizer.isEmpty()) {
		...
		// If the sequencer were also a synthesizer, that would make things easy:
	...
	private MidiDevice openMidiDevice(String containsString) {
		...
		for (MidiDevice.Info midiDevice : midiDevices) {
			...
			if (device != null && midiDevice.getName().contains(
					containsString)) {
		...
		public void run() {
			while (!dying()) {
				...
				if (fadeOutGain == -1.0f && fadeInGain == 1.0f)
					snooze(3600000);
				checkFadeOut();
				// only update every 50 milliseconds (no need to peg the cpu)
				snooze(50);
			}
			// Important!
			cleanup();
