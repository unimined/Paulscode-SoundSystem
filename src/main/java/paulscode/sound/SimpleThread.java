 ...
 * like sleeping, killing, and checking aliveness.  NOTE: super.cleanup() must
 * be called at the bottom of overridden cleanup() methods, and cleanup()
...
public class SimpleThread extends Thread {
	 ...
	 * completed.  NOTE: super.cleanup() must be called at the bottom of overridden
	...
	/**
	 * Calls the {@link #rerun()} method on a separate thread,
	 * which calls {@link #run()} when the previous thread finishes.
	 */
	public void restart() {
	...
	/**
	 * @return true when the thread is supposed to shut down.
	 */
	...
	private synchronized boolean alive(boolean action, boolean value) {
		if (action) alive = value;
		return alive;
	}

	/**
	 * Sets or returns the value of {@link #kill}.
	 *
	 * @param action GET or SET.
	 * @param value  New value if action == SET, or XXX if action == GET.
